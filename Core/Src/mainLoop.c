...
//tim4 dma 方式，获得数据分为两部分，分为前后两个halfword，每个halfword都代表一个上升沿的时间
// 这段代码是用来处理这两个halfword的
//输入参数 canNum为两个halfword的组合数，有上个上升沿时间, 由于第一个数通常是错误的，所以从第二个数开始
void transform_capNum_to_xNum(uint32_t* capNum, int32_t* xNum, int32_t capNum_len){
  int32_t i;
...
void enable_tim4_capture_Iterrupt(void){
 // HAL_TIM_Base_Start_IT(&htim4);
  ...
  //HAL_TIM_IC_Start_DMA(&htim4, TIM_CHANNEL_1, (uint32_t*)chanl1_capNum, CAPNUM+4);
  
  // HAL_TIM_IC_Start_DMA(&htim4, TIM_CHANNEL_2, (uint32_t*)chanl2_capNum, CAPNUM);
  // HAL_TIM_IC_Start_DMA(&htim1, TIM_CHANNEL_4, (uint32_t*)chanl1_capNum, CAPNUM);
...
//	}
 ...
 // HAL_GPIO_TogglePin(LED3_GPIO_Port, LED3_Pin);
  HAL_DAC_SetValue(&hdac, DAC_CHANNEL_1, DAC_ALIGN_12B_R, (uint32_t)((v_dianya/3.3)*4096));
  ...
  while(1){
   ...
   HAL_Delay(3);
    ...
    while(1){
      ...
      //   chanl1_completed=0;
      //   transform_capNum_to_xNum((uint32_t *)(&chanl1_capNum[2]), chanl1_xNum, CAPNUM);
      //   average_xNum1 = (float)cal_average_xNum(chanl1_xNum, CAPNUM);
      //   //HAL_TIM_IC_Start_DMA(&htim4, TIM_CHANNEL_1, (uint32_t*)chanl1_capNum, CAPNUM);
...
uint32_t value_dacP=0x800000,value_dacN=0x800000;
...
void mainLoop1(void){
	...
	int32_t runTimes = 0; //log 30s, for example 0.5ms*2000=1s, runtimes=2000*60=1min
	int32_t xyz=0;
